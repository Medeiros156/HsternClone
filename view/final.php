...
$clothePurchased = $clotheController->getclothe($_GET['clothe_id']);
//realiza a "compra"
$clotheController->buy($clothePurchased['title']);

// echo print_r($clothePurchased['title']);
...
<body>
	...
	<div id="final" class="container">
		<div class="clothe-card">
			<!-- <img src="<?= $clothePurchased['img'][0] ?>" alt="<?= $clothePurchased['title'] ?>" /> -->
			<div class="image-container">

				<?= '<img src="' . $clothePurchased['img'][0] . '" alt="' . $clothePurchased['title'] . '" />'; ?>
				<?= '<img src="' . $clothePurchased['img'][1] . '" alt="' . $clothePurchased['title'] . '" class="hover-image" />'; ?>

			</div>
			<h5>
				<?= $clothePurchased['title'] ?>
			</h5>


			<p>
				<?= $clothePurchased['price'] ?>
			</p>
			<?php
			if ($functions->alreadyPurchased($clothePurchased['title'])) {
				echo "<p class='purchased'>Comprado</p>";
			} else {
				echo "<a href='?p=final&clothe_id=" . $clothePurchased['title'] . "'>Comprar</a>";
			}
			?>
		...
		<div class="purchased-info">
			...
			<p>O item <strong>
					<?= $clothePurchased['title'] ?>
				</strong> foi adicionado a sua lista de compras.</p>
